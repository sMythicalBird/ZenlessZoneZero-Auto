...
def daily_task() -> None:
    ...
    def get_click_pos(title):  # 获取任务对应点击位置
        for i in [x for x in ocr_results if x.text == goto_text]:
            if (
                i.position.x1 > daily_tasks[title].x1
                and i.position.x1 < daily_tasks[title].x2
            ):
                return i.position
    ...
    def exit_task():  # 退出到快捷手册界面
        while "快捷手册" not in [x.text for x in ocr_results if x.position.y > 680]:
            control.esc(duration=0.1)
            time.sleep(1)
    ...
    def daily_task_coffee():  # 品尝1次咖啡
        ...
        if "点单" in [x.text for x in ocr_results]:
            if "新艾利都特调" in [
                ...
                click_pos = [x for x in ocr_results if x.text == "新艾利都特调"][
                    0
                ].position
    ...
    for processing_task in [
        x for x in ocr_target_pos_list if x.text == unfinished_text
    ]:
        for daily_task in daily_tasks:
            if (
                processing_task.position.x > daily_tasks[daily_task].x1
                and processing_task.position.x < daily_tasks[daily_task].x2
            ):
